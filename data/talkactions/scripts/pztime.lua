function onSay(player, words, param)
    ...
    if fragTime <= 0 then
        player:sendTextMessage(MESSAGE_EVENT_DEFAULT, "You do not have any unjustified kill.")
        --return false
    end

    local skullTime = player:getSkullTime()
    print(player:getSkullTime(0), player:getSkullTime(1), player:getSkullTime(2), player:getSkullTime(3))
    if skullTime <= 0 then
        player:sendTextMessage(MESSAGE_EVENT_DEFAULT, "You do not have any unjustified kill.")
        --return false
    end
