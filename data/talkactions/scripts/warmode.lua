function onSay(player, words, param)
	...
	if check then
		return player:sendCancelMessage('Voce ja aceitou uma soliticacao de guerra, aguarde o fim da war para declarar uma nova guerra.')
	end
	if check2 then
		return player:sendCancelMessage('Sua solicitacao de guerra foi aceita, aguarde o fim da war para declarar uma nova guerra.')
	end
	result.free(check)
	result.free(check2)
