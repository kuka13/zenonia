    ...
    if modalWindowId == 1840 then
		if buttonId == 105 then
			...
			if guildId == warmodet1 then
				...
				if contain == 0 then
					...
					return player:sendCancelMessage('So e permitido entrar uma vez na war.')
				end
			end
			...
			player:sendCancelMessage('Boa sorte.')
		...
		if buttonId == 103 then
			...
			if i > 1 then
				...
				return player:sendCancelMessage('Nao ha solicitacoes para serem aceitadas.')
			end
		...
		if buttonId == 104 then
			...
			if aux > 0 then
				...
				return player:sendCancelMessage('Nao ha solicitacoes para serem removidas.')
			end
		end
        ...
        if buttonId == 102 then
			...
			if i > 1 then
				...
				return player:sendCancelMessage('nao ha guilds para desafiar')
			end
		end
		...
		if buttonId == 103 then
			...
			if #warmodeagenda >= 23 then
				return player:sendCancelMessage('Agenda de horarios cheia.')
			end
		...
		if not list then
			...
			return player:sendCancelMessage('Voce ja desafiou essa guild')
		end
		result.free(list)
		...
		return player:sendCancelMessage('A solicitacao foi deletada com sucesso.')
		...
		if buttonId == 100 then
			...
			checkWarmode()
			return player:sendCancelMessage('A solicitacao foi aceita com sucesso. >>Para facilitar os testes tentaremos iniciar a war agora<< .. Digite !war para entrar')
