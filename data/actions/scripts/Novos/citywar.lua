...
function CityWarsonInviteExpired(invite)
...
function CityWarsonInvite(city, inviter, invitedGuild, invitedLeaders, options)
    ...
    invite.expireEvent = addEvent(CityWarsonInviteExpired, 120 * 1000, invite)
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    window:addButton('Ok')
        ...
        elseif accept and choice == accept then
            ...
            invitesWindow:addButton('Ok')
                if choice then
                    CityWarsonInviteAccepted(player, choice.invite)
                end
