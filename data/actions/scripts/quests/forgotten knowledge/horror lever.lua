local config = {
	requiredLevel = 200,
	daily = true,
	BossPosition = Position(32269, 31087, 14),
	MinosPosition = Position(32269, 31086, 14),
	centerDemonRoomPosition = Position(32268, 31090, 14),
	playerPositions = {
		Position(32302, 31088, 14), -- Posição dos players 1,2,3,4,5
	...
	newPositions = {
		Position(32265, 31098, 14),
		Position(32266, 31098, 14),
		Position(32267, 31098, 14),
		Position(32268, 31098, 14),
		Position(32269, 31098, 14)
	},
}
...
function onUse(player, item, fromPosition, target, toPosition, monster, isHotkey)
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if not (player:getStorageValue(49084) <= os.time()) then
		...
		for i = 1, #specs do
			...
			if spec:isPlayer() then
				if item.itemid == 9826 then
					if player:getPosition() ~= Position(32269, 31089, 14) then
						item:transform(9826)
		...
		local players
		local monster = Game.createMonster("melting frozen horror", Position(32269, 31087, 14))
		monster:setReward(true)
		for i = 1, #storePlayers do
			players = storePlayers[i]
		
			if not (players:getStorageValue(49084) <= os.time()) then
		...
		for i = 1, #storePlayers do
			...
			player:setStorageValue(49084, os.time()+exaustedSeconds*72000)
		end
