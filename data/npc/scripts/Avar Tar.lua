...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == 2 then
		...
		elseif msgcontains(msg, 'shield') then
			if player:addOutfitAddon(541, 1) then
		...
		elseif msgcontains(msg, 'helmet') then
			if player:addOutfitAddon(541, 2) then
				...
				npcHandler:say('The helmet is for those who have hunted down all 666 demons and finished the demon oak as well, |PLAYERNAME|.', cid)
				npcHandler.topic[cid] = 2
			end
		end
	end
